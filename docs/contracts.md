...
All dependencies are exact versions, and CI installs the lockfile frozen. The root-level `pnpm-workspace.yaml` overrides are also an audited dependency contract; they currently pin Next's transitive dependencies to `postcss 8.5.25` and `sharp 0.35.4`, to fix the known vulnerabilities that the 2026-08-03 audit hit (`sharp` was raised from `0.35.0` on 2026-09-22 with `next 16.3.3` and `fastify 5.12.1`, after new advisories against the earlier pins). Upgrades are committed separately, and MUST pass typecheck, production audit, migration on an empty database and on a repeated run, schema drift, fixtures, licensing, and the Compose smoke test. Generated JSON Schema/OpenAPI are committed artifacts; CI fails when the source and the generated artifacts disagree.
